...
#include "mikiri_waf_ws_module.h"
...
static ngx_int_t ngx_http_waf_ws_init_process(ngx_cycle_t *cycle);
static void ngx_http_waf_ws_exit_process(ngx_cycle_t *cycle);
...
u_char waf_ws_close[2] = {0x88, 0x00};
...
static ngx_command_t  ngx_http_waf_ws_commands[] = {
    ...
    { ngx_string("waf_ws_proxy_pass"),
      NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_HTTP_LMT_CONF|NGX_CONF_TAKE1,
      ngx_http_waf_proxy_pass,
...
ngx_module_t  mikiri_waf_ws_module = {
    ...
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    ngx_http_waf_ws_init_process,         /* init process */
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    ngx_http_waf_ws_exit_process,         /* exit process */
    NULL,                                  /* exit master */
...
static ngx_int_t ngx_http_waf_ws_init_process(ngx_cycle_t *cycle){
  ...
  wwsmc = ngx_http_cycle_get_module_main_conf(cycle, mikiri_waf_ws_module);

  waf_memc_connect(wwsmc);
    ...
    ngx_uint_t from_upstream, ngx_uint_t do_write)
{
	...
	wmc = ngx_http_get_module_main_conf(r, mikiri_waf_http_module);
    ws_rctx = ngx_http_get_module_ctx(r, mikiri_waf_ws_module);
                ...
                if (ws_rctx != NULL && wmc != NULL && ws_rctx->ws_request == 1) {
                  if (waf_check_flags(ws_rctx->rctx, r, wmc) == NGX_OK) {
                    if (!from_upstream){
                      ...
                      waf_process_response(b, ws_rctx, r);
                      waf_process_upstream(b, ws_rctx, r);
...
ngx_http_upstream_process_headers(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for (i = 0; /* void */; i++) {
        ...
        waf_process_header(h[i], r); // Process headers for extensions
...
static void
ngx_http_waf_upstream_init_request(ngx_http_request_t *r)
{
    ngx_str_t                      *host;
    ngx_uint_t                      i;
    ...
    if (u->conf->cache) {
        ...
        if (rc == NGX_BUSY) {
            r->write_event_handler = ngx_http_waf_upstream_init_request;
            return;
        }
...
// Create request context
    waf_create_ws_rctx(r);
//
    ngx_http_upstream_connect(r, u);
}

void ngx_http_waf_upstream_init(ngx_http_request_t *r)
{
    ngx_connection_t     *c;
...
#if (NGX_HTTP_V2)
    if (r->stream) {
        ngx_http_waf_upstream_init_request(r);
        return;
    }
#endif
...
static ngx_int_t ngx_http_waf_proxy_handler(ngx_http_request_t *r)
{
    ngx_int_t                    rc;
    ngx_http_upstream_t         *u;
    ...
    rc = ngx_http_read_client_request_body(r, ngx_http_waf_upstream_init);
...
ngx_http_waf_ws_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    {
        clcf->handler = ngx_http_waf_proxy_handler;
    }
...
static char * ngx_http_waf_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
